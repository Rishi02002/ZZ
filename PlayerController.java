...
public class PlayerController {
    ...
    public boolean canBuildVillage() {
        // TODO: H2.5
        // Check if the player's current goal is to place a village
        boolean isGoalToPlaceVillage = this.playerObjectiveProperty.getValue() == PlayerObjective.PLACE_VILLAGE;
        // Check if the player has the required resources to build a village
        boolean hasRequiredResources = this.player.hasResources(Config.SETTLEMENT_BUILDING_COST.get(Settlement.Type.VILLAGE));
        // Check if the player has sufficient settlements to build
        boolean hasSettlements = this.player.getRemainingVillages() > 0;

        return (isGoalToPlaceVillage || hasRequiredResources) && hasSettlements;
    }
    ...
    public void buildVillage(final Intersection intersection) throws IllegalActionException {
        // TODO: H2.5
        // First check whether the player can build a village
        if(!canBuildVillage())
            throw new IllegalActionException("The player " + this.player.getName() + " can't build a village yet");

        // Check if the player can place a village at the specified intersection
        if(!intersection.placeVillage(this.player, isFirstRound())) {
            throw new IllegalActionException("The player " + this.player.getName()
                + " has no road lead to the intersection or already a settlement on it!");
        }

        // Check if the player's current goal is to place a village
        boolean isGoalToPlaceVillage = this.playerObjectiveProperty.getValue() == PlayerObjective.PLACE_VILLAGE;
        if(!isGoalToPlaceVillage)
            // TODO: why remove resources from player wenn player's current goal is not to place a village? Make no sense!
    ...
    public void upgradeVillage(final Intersection intersection) throws IllegalActionException {
        // TODO: H2.6
        // Check whether the player is allowed to upgrade a village to a city
        if(!this.canUpgradeVillage())
            throw new IllegalActionException("The player " + this.player.getName() + " can not upgrade the village on intersection: " + intersection.toString());

        // Upgrade the village and remove the needed resources from the player
        if(intersection.upgradeSettlement(this.player))
            // Remove the required resources from the player if conditions are met
            this.player.removeResources(Config.SETTLEMENT_BUILDING_COST.get(Settlement.Type.CITY));
        else
            throw new IllegalActionException("The player has no village or other settlement on the intersection : " + intersection.toString());
    }
    ...
    public boolean canBuildRoad() {
        // TODO: H2.5
        // Check if the player's current goal is to place a road
        boolean isGoalToPlaceRoad = this.playerObjectiveProperty.getValue() == PlayerObjective.PLACE_ROAD;
        // Check if the player has the required resources to build a road
        boolean hasRequiredResources = this.player.hasResources(Config.ROAD_BUILDING_COST);
        // Check if the player has enough roads to build
        boolean hasSettlements = this.player.getRemainingRoads() > 0;

        return (isGoalToPlaceRoad || hasRequiredResources) && hasSettlements;
    }
    ...
    public void buildRoad(final TilePosition position0, final TilePosition position1) throws IllegalActionException {
        // TODO: H2.5
        // First check whether the player is allowed to build a road
        if(!this.canBuildRoad())
            throw new IllegalActionException("The player "+ player.getName() +" can not build a road!");

        // Set the road and check if the road can be built
        if(!player.getHexGrid().addRoad(position0, position1, player, isFirstRound()))
            throw new IllegalActionException("The player " + player.getName() + " can not add a road on the edge!");

        // Check if the player's current goal is to place a road
        boolean isGoalToPlaceRoad = this.playerObjectiveProperty.getValue() == PlayerObjective.PLACE_ROAD;
        if(!isGoalToPlaceRoad)
    ...
    throws IllegalActionException {
        // TODO: H2.3
        // Check if the player has the offered resources
        Map<ResourceType, Integer> offeredResources = new HashMap<>();
        offeredResources.put(offerType, offerAmount);
        boolean hasOfferedResources = this.player.hasResources(offeredResources);

        // Check first the player's trade ratio for the offered resource type
        boolean isTradeRatioPassed = this.player.getTradeRatio(offerType) <= offerAmount;

        if(!isTradeRatioPassed || !hasOfferedResources)
            throw new IllegalActionException("The player " + player.getName()+ " offers an insufficient amount of resources for type: " + offerType);

        // Otherwise, remove offered resource with offerAmount and add 1 requested resource in player's account
    ...
    public void acceptTradeOffer(final boolean accepted) throws IllegalActionException {
        // TODO: H2.3
        // First check whether a trading offer and request exists.
        if(this.playerTradingOffer == null || this.playerTradingRequest == null)
            throw new IllegalActionException("No trade offer to accept");

        // Check whether both players have enough resources for the trade
        // Check whether first this player him/herself has enough resources
        if(!this.canAcceptTradeOffer(this.tradingPlayer, this.playerTradingRequest))
            throw new IllegalActionException("Player does not have the requested resources");
        // Check whether other player is not null and has enough resources
        if(tradingPlayer == null || !tradingPlayer.hasResources(this.playerTradingOffer))
            throw new IllegalActionException("Other player does not have the offered resources");

        // If the "accepted" is false, the trade is rejected.
        if(!accepted){
        ...
        player.addResources(this.playerTradingOffer);
        player.removeResources(this.playerTradingRequest);

        tradingPlayer.addResources(this.playerTradingRequest);
        tradingPlayer.removeResources(this.playerTradingOffer);

        // Set the objective of the current player to PlayerObjective.IDLE
        this.setPlayerObjective(PlayerObjective.IDLE);
    }
